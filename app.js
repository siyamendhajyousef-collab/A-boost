/* ==========================================================================
   Ag Boost Ultimate - Main Application Logic (app.js)
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (refCode) {
        ...
        if (regInput) {
            ...
            localStorage.setItem('ag_ref_code', refCode.toUpperCase());
        }
        switchAuthTab('register');
    } else if (localStorage.getItem('ag_ref_code')) {
        const regInput = document.getElementById('regReferralCode');
        if (regInput) {
            regInput.value = localStorage.getItem('ag_ref_code');
        }
    }
        ...
        return `
            <div class="glass-card p-5 rounded-3xl border relative overflow-hidden bg-gradient-to-br ${tier.badgeColor} shadow-xl space-y-4">
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-300">ترقية حصرية</span>
                        <h3 class="text-lg font-black text-white">${tier.name}</h3>
                    </div>
                    <div class="text-left">
                        <span class="text-2xl font-black text-amber-400">${tier.price}</span>
                        <span class="text-xs font-bold text-slate-300"> USDT</span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 text-xs border-y border-slate-800/80 py-3">
                    <div class="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50">
                        <span class="text-slate-400 block text-[10px]">المهام اليومية</span>
                        <b class="text-white text-sm">${tier.tasks} مهمة/يوم</b>
                    </div>
                    <div class="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50">
                        <span class="text-slate-400 block text-[10px]">الربح اليومي المتوقع</span>
                        <b class="text-emerald-400 text-sm">$${tier.dailyProfit.toFixed(2)}</b>
                    </div>
                    <div class="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50">
                        <span class="text-slate-400 block text-[10px]">الربح الشهري</span>
                        <b class="text-emerald-400 text-sm">$${tier.monthlyProfit.toFixed(2)}</b>
                    </div>
                    <div class="bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/50">
                        <span class="text-slate-400 block text-[10px]">الربح السنوي</span>
...
async function sendAiMessage() {
    ...
    chatBox.innerHTML += `<div id="${loadingId}" class="bg-slate-900 p-3 rounded-xl border border-slate-800 text-slate-400 animate-pulse">Ag AI: جاري التفكير والتحليل...</div>`;
    chatBox.scrollTop = chatBox.scrollHeight;
    ...
    try {
        ...
        if (res.ok && data.reply) {
            loadingEl.outerHTML = `<div class="bg-slate-900 p-3 rounded-xl border border-slate-800 text-slate-300">Ag AI: ${data.reply}</div>`;
            playBeep('win');
        } else {
            loadingEl.outerHTML = `<div class="bg-rose-950/40 p-3 rounded-xl border border-rose-900 text-rose-300">Ag AI: ${data.error || 'عذراً، حدث خطأ في معالجة الطلب.'}</div>`;
        }
    } catch (err) {
        const loadingEl = document.getElementById(loadingId);
        if (loadingEl) {
            loadingEl.outerHTML = `<div class="bg-rose-950/40 p-3 rounded-xl border border-rose-900 text-rose-300">Ag AI: تعذر الاتصال بالخادم.</div>`;
        }
    }
    chatBox.scrollTop = chatBox.scrollHeight;
}
